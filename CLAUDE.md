...
- Server page (`statistics/page.tsx`) reads `searchParams.year` via `resolveSeasonParam`, fetches `getScoreTrends(season)` + `getCourseBreakdowns(season)` + `getLeagueYearlyAverages()` + `getTopRoundsAllTime(5)` (last two are NOT season-scoped — pull all years), renders heading row (`<h1>Statistics</h1>` + `<YearDropdown />`) + green divider + `<StatisticsClient />` body + footer "Our History" pill
- Heading lives in the page, not the client component — `StatisticsClient` is body-only and starts at the course filter pills
- `StatisticsClient` is a `"use client"` component owning `selectedCourse` state; the season-scoped sections (summary cards, Season History, Course Breakdown, Score Distribution, Top 5 — selected season) react to the course filter; the cross-season sections (Season Averages by Year, Top 5 — All Time) do not. Props: `{ trends, courseBreakdowns, yearlyAverages, allTimeTopRounds, selectedYear, playersHref }`
...
- Section order: Season History → Course Breakdown (All only) → Score Distribution → Season Averages by Year → Top 5 Best Rounds (selected season) → Top 5 Best Rounds (All Time) → footer note. The "Our History" pill below the body lives in the page wrapper.
- Footer note: styled box (`rounded-lg border bg-muted/40 px-4 py-3`) with bolded "course filters at the top"
- Top 5 Best Rounds tables (two stacked cards, both with the same column layout): muted uppercase header, columns: # · Player · Course (color-coded) · Date · Score (green)
  - **Selected-season** card title: `Top 5 Best Rounds — {selectedYear}` with optional ` · {Course}` suffix when a course pill is active. Data is filtered + sorted client-side from `trends`.
  - **All-Time** card title: `Top 5 Best Rounds — All Time`. Data comes from `getTopRoundsAllTime(5)` (server-fetched, ignores selected year and course filter). Date column shows `MMM D, YYYY` (year included to disambiguate cross-season).
...
- Section order: Season History chart → Course Breakdown tiles (All only) → Score Distribution → Season Averages by Year → Top 5 Best Rounds (All Time) → Round History table
- Top 5 Best Rounds — All Time card (player-scoped, 4 columns: # · Course · Date · Score): server-fetched via `getPlayerTopRoundsAllTime(slug, 5)`; ignores selected year and course filter. Date column shows `MMM D, YYYY` (year included to disambiguate cross-season).
- Round History table title reads `Round History — {selectedYear}` with optional ` · {Course}` suffix when a course pill is active.
